...
public class HerdSpawner : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = 0; i < 2; i++) {
			goats[counter] = Instantiate(goat,new Vector3(transform.position.x-i*xOffset+xOffset*0.5f,0,transform.position.z), Quaternion.identity);
		...
		for (int i = 0; i < 3; i++) {
			goats[counter] = Instantiate(goat,new Vector3(transform.position.x-i*xOffset,0,transform.position.z-zOffset), Quaternion.identity);
			goats[counter].GetComponent<GoatController> ().bounceScale = Random.Range (1, 4);
			goats[counter].GetComponent<GoatController> ().step = Random.Range (0.1f, 0.3f);
			goats[counter].GetComponent<GoatController> ().timer = Random.Range (0.0f, Mathf.PI);
			goats[counter].GetComponent<GoatController> ().speed = speed;
		...
		for (int i = 0; i < 2; i++) {
			goats[counter] = Instantiate(goat,new Vector3(transform.position.x-i*xOffset+xOffset*0.5f,0,transform.position.z-zOffset*2), Quaternion.identity);
			goats[counter].GetComponent<GoatController> ().bounceScale = Random.Range (1, 4);
			goats[counter].GetComponent<GoatController> ().step = Random.Range (0.1f, 0.3f);
			goats[counter].GetComponent<GoatController> ().timer = Random.Range (0.0f, Mathf.PI);
			goats[counter].GetComponent<GoatController> ().speed = speed;
